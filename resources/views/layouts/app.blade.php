...
<head>
    ...
    <title>@yield('title', 'RaffaCCTV System')</title>
    ...
    <div class="flex min-h-screen">
        ...
        <aside class="w-72 bg-white border-r border-slate-200 flex flex-col fixed h-full z-50">
            <div class="p-8 flex items-center gap-3">
                ...
                <h2 class="text-xl font-bold tracking-tight text-slate-800">Raffa<span class="text-blue-600">CCTV</span></h2>
            </div>
            ...
            <nav class="flex-1 px-6 space-y-1">
                ...
                <a href="/dashboard" class="flex items-center px-4 py-3 rounded-2xl transition-all {{ request()->is('dashboard') ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'text-slate-500 hover:bg-slate-50' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                    <span class="font-semibold text-sm">Dashboard</span>
                </a>

                <a href="/users" class="flex items-center px-4 py-3 rounded-2xl transition-all {{ request()->is('users*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'text-slate-500 hover:bg-slate-50' }}">
                ...
                <a href="/alat/create" class="flex items-center px-4 py-3 rounded-2xl transition-all {{ request()->is('alat/create') ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'text-slate-500 hover:bg-slate-50' }}">
                    ...
                    <span class="font-semibold text-sm">Create Alat</span>
                </a>

                <a href="/repair" class="flex items-center px-4 py-3 rounded-2xl transition-all {{ request()->is('repair') ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'text-slate-500 hover:bg-slate-50' }}">
                ...
                <a href="/report" class="flex items-center px-4 py-3 rounded-2xl transition-all {{ request()->is('report') ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'text-slate-500 hover:bg-slate-50' }}">
        ...
        <div class="flex-1 ml-72 flex flex-col min-h-screen">
            <header class="h-16 border-b border-slate-100 bg-white/50 backdrop-blur-md sticky top-0 z-40 flex items-center px-10">
                <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Raffa CCTV Monitoring System</div>
            </header>
            ...
            <footer class="p-8 text-center text-slate-400 text-[10px] font-medium tracking-widest border-t border-slate-100">
                &copy; 2026 CRAFTED BY <span class="text-slate-900 font-bold italic underline decoration-blue-500">PT.RAFFA CCTV</span>
            </footer>
        </div>
        @else
            <div class="w-full">
